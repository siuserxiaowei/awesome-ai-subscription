# awesome-ai-subscription · 中文 AI 订阅完整指南

<!-- SIUSER-REPO-GUIDE:START -->
## 项目介绍 / Project Introduction

### 中文
AI 订阅产品清单：整理 AI SaaS、订阅工具和产品参考。

### English
Awesome AI subscription list for AI SaaS products, subscription tools, and product references.

## 使用方式 / Usage

### 中文
1. 优先打开在线入口或本地静态服务查看最终页面。
2. 内容型仓库通常从 `README.md`、`docs/`、`data/` 或 `content/` 开始阅读。
3. 更新资料后，重新生成或刷新静态页面，并检查链接、图片和文字是否正常。

### English
1. Start with the live link or a local static server to view the final page.
2. For content repositories, begin with `README.md`, `docs/`, `data/`, or `content/`.
3. After updating material, regenerate or refresh the static page and check links, images, and copy.

## 入口与元信息 / Entry Points & Metadata

- GitHub 仓库 / Repository: https://github.com/siuserxiaowei/awesome-ai-subscription
- Live / 在线入口：https://payforgpt.com
- 默认分支 / Default branch: `main`
- 主要语言 / Primary language: `project`
- 可见性 / Visibility: `public`
- 仓库类型 / Repository type: `source`
- Topics / 主题：`ai-subscription`, `awesome`, `awesome-list`, `chatgpt`, `chatgpt-plus`, `chinese`, `claude`, `claude-pro`, `gemini`, `guide`

## 本地运行 / Local Run

```bash
git clone https://github.com/siuserxiaowei/awesome-ai-subscription.git
cd awesome-ai-subscription
python3 -m http.server 8000
```

## 仓库结构 / Repository Map

| 路径 / Path | 中文说明 | English |
| --- | --- | --- |
| `README.md` | 项目入口说明，先读这里。 | Main project entry point and orientation. |
| `assets` | 图片、样式、数据等资源。 | Images, styles, data, and other assets. |
| `docs` | 文档或 GitHub Pages 输出目录。 | Documentation or GitHub Pages output. |
| `.github` | GitHub Actions 和协作自动化配置。 | GitHub Actions and collaboration automation. |
| `LICENSE` | 许可证文件。 | License file. |
| `.gitignore` | 项目文件或目录。 | Project file or directory. |
| `.lycheeignore` | 项目文件或目录。 | Project file or directory. |
| `.markdownlint.json` | 项目文件或目录。 | Project file or directory. |
| `CONTRIBUTING.md` | 项目文件或目录。 | Project file or directory. |
| `README.en.md` | 项目文件或目录。 | Project file or directory. |

## 维护备注 / Maintenance Notes

- 中文：当项目目标、在线入口、运行命令或目录结构变化时，同步更新本说明。
- English: Keep this guide updated when the project purpose, live link, run commands, or structure changes.
- 中文：修改代码、数据或生成页面后，优先运行相关构建、测试或校验命令。
- English: After changing code, data, or generated pages, run the relevant build, test, or validation command.

## 安全与隐私 / Safety & Privacy

- 中文：不要提交 API key、token、密码、cookie、私有链接或内部账号资料。
- English: Do not commit API keys, tokens, passwords, cookies, private URLs, or internal account data.
- 中文：公开 GitHub Pages 前，确认资料已脱敏并允许公开。
- English: Before publishing GitHub Pages output, confirm the material is redacted and cleared for public release.
<!-- SIUSER-REPO-GUIDE:END -->



<!-- SIUSER-SEO-INTRO:START -->

## 项目介绍 / Project Introduction

**中文介绍**：AI 订阅产品与工具清单，整理值得关注的 AI SaaS、订阅服务、工具栈和创作者资源。

**English**: A curated list of AI subscription products and tools, covering AI SaaS, subscriptions, tool stacks, and creator resources.

**SEO 关键词 / SEO Keywords**: awesome list, AI SaaS, AI subscription, AI tools, 工具清单

<!-- SIUSER-SEO-INTRO:END -->

[English](README.en.md) | [中文]

---

[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)
[![GitHub stars](https://img.shields.io/github/stars/siuserxiaowei/awesome-ai-subscription?style=social)](https://github.com/siuserxiaowei/awesome-ai-subscription/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/siuserxiaowei/awesome-ai-subscription?style=social)](https://github.com/siuserxiaowei/awesome-ai-subscription/network/members)
[![Last commit](https://img.shields.io/github/last-commit/siuserxiaowei/awesome-ai-subscription)](https://github.com/siuserxiaowei/awesome-ai-subscription/commits/main)
[![License](https://img.shields.io/github/license/siuserxiaowei/awesome-ai-subscription)](./LICENSE)

> 一份持续维护的中文 AI 订阅手册：ChatGPT、Claude、Gemini、Grok、Cursor、Codex 各家档位、价格、功能差异、付款方式、风控规避、卡密激活、按场景的组合推荐——全部用具体数字和实战经验,不堆 AI 套话。

国内付订阅是个又琐碎又坑的事:海外信用卡可能被拒,Apple ID 礼品卡可能被冻结,IP 异常会触发风控,卡密发错号能让你折腾一晚上。这份指南是写给所有"看完官方页面还是不知道该买哪档"和"卡在付款页一小时"的中文用户的。每一页都基于实测,数字精确到月费 $0.01,案例都是真实场景里复盘出来的处方。如果你只想知道"该订哪家",直接跳到 [09-按场景推荐](./docs/09-recommendation-by-use-case.md)。

---

## Quick Links · 高频问题

- [ChatGPT Plus 多少钱?Pro 值不值 $200/月?](./docs/01-chatgpt-complete-guide.md#档位与价格)
- [Claude Pro 怎么订?Max $100 跟 $200 差多少?](./docs/02-claude-complete-guide.md#档位对比)
- [国内信用卡能直接付订阅吗?为什么常被拒?](./docs/06-payment-methods.md#海外信用卡-国内卡)
- [代充靠谱吗?怎么挑不被骗?](./docs/06-payment-methods.md#代充服务)
- [一个月预算 $20 / $50 / $200,各家怎么组合?](./docs/07-pricing-comparison.md#预算组合推荐)
- [付款失败怎么办?礼品卡被冻结怎么解?](./docs/08-troubleshooting.md)

---

## English version

完整英文版已上线 → 见 [README.en.md](./README.en.md) 与 [docs/en/](./docs/en/)。10 篇逐篇翻译,每篇英文版顶部 / 底部都有"中文版"双向跳转。

---

## 目录

### 核心订阅指南

| #   | 文档                                                                   | 一句话简介                                                          |
| --- | ---------------------------------------------------------------------- | ------------------------------------------------------------------- |
| 01  | [ChatGPT 完整订阅指南](./docs/01-chatgpt-complete-guide.md)            | Free / Plus $20 / Pro $200 / Team / Enterprise 全档位与国内订阅路径 |
| 02  | [Claude 完整订阅指南](./docs/02-claude-complete-guide.md)              | Free / Pro $20 / Max $100 / Max $200 + Console API 怎么选           |
| 03  | [Gemini Advanced 指南](./docs/03-gemini-complete-guide.md)             | Google AI Pro $19.99 / AI Ultra,跟 ChatGPT Plus 谁更值              |
| 04  | [Grok Premium 指南](./docs/04-grok-complete-guide.md)                  | Premium $8 / Premium+ $40 / xAI API + X 订阅捆绑机制                |
| 05  | [Cursor / Codex 编程订阅](./docs/05-cursor-codex-guide.md)             | Cursor Pro $20 / Business $40 / OpenAI Codex,程序员怎么挑           |

### 付款 / 选择 / 排错

| #   | 文档                                                                       | 一句话简介                                                |
| --- | -------------------------------------------------------------------------- | --------------------------------------------------------- |
| 06  | [付款方式全方位对比](./docs/06-payment-methods.md)                         | 海外卡 / Apple Pay / 礼品卡 / 虚拟卡 / USDT / 代充 7 种   |
| 07  | [各家定价矩阵 + 性价比](./docs/07-pricing-comparison.md)                   | 1 张大表横向对比,按预算 $20/$50/$100/$200 推组合          |
| 08  | [付款失败 / 风控 / 卡密 故障排查](./docs/08-troubleshooting.md)            | 10+ 真实场景,每条 2-3 步处方                              |
| 09  | [按场景选订阅(程序员/学生/写作者/研究者/普通用户)](./docs/09-recommendation-by-use-case.md) | 5 类用户的月预算 + 推荐组合                |
| 10  | [追踪政策变化](./docs/10-stay-updated.md)                                  | 各家官方 status / changelog / newsletter / Discord 清单   |

---

## 主流订阅一图速览

| 服务                | 月费 (USD)  | 主要功能                                                 | 适合场景                       | 国内订阅难度 |
| ------------------- | ----------- | -------------------------------------------------------- | ------------------------------ | ------------ |
| ChatGPT Plus        | $20         | GPT-5 / o-系列推理模型 / 高级语音 / DALL·E / Code Interpreter | 普通用户、写作、轻度编程       | 中           |
| ChatGPT Pro         | $200        | 无限 GPT-5 Pro / o3-pro 高强度推理 / Sora 优先额度           | 重度研究者、科研、长链推理     | 高           |
| Claude Pro          | $20         | Claude Sonnet/Opus + Projects + 5x 用量                  | 写作、长文档分析、代码         | 中           |
| Claude Max ($100)   | $100        | 5x Pro 用量、优先访问新模型、Claude Code 配额提升         | 重度长上下文、Agent 开发       | 高           |
| Claude Max ($200)   | $200        | 20x Pro 用量、最高优先级、Claude Code 几乎不限速           | 高强度 Agent、企业级编码       | 高           |
| Gemini Advanced (AI Pro) | $19.99 | Gemini 2.5 Pro + 2TB 云盘 + Workspace 集成 + Veo 视频生成 | Google 重度用户、视频内容创作 | 中           |
| Grok Premium        | $8          | Grok 4 + X 高级功能(去广告、长贴等)                      | X 重度用户、追实时新闻         | 低-中        |
| Cursor Pro          | $20         | 无限 Tab 补全 + 500 次高级请求 / 月                       | 程序员日常编辑器               | 中           |

> 价格以官方美区为准,2026-05 数据。各家定期调价,实时价格请翻 [07-定价对比](./docs/07-pricing-comparison.md)。

---

## 怎么用这份指南

- **第一次看 AI 订阅** → 先翻 [09-按场景推荐](./docs/09-recommendation-by-use-case.md),找到自己那一类
- **已经知道要订哪家、卡在付款** → [06-付款方式](./docs/06-payment-methods.md) + [08-故障排查](./docs/08-troubleshooting.md)
- **预算紧、想最大化性价比** → [07-定价矩阵](./docs/07-pricing-comparison.md) 看预算组合
- **想长期跟政策** → [10-追踪政策](./docs/10-stay-updated.md) 列了官方 changelog 和优质信息源

---

## 贡献

发现错误、价格变动、新档位、付款新姿势,都欢迎 PR。看一眼 [CONTRIBUTING.md](./CONTRIBUTING.md) 30 秒就懂规则。

最受欢迎的贡献类型:
- 价格更新(各家每年至少调价 1-2 次)
- 国家/地区限制变化(例:Gemini 在某些地区开放/关闭)
- 新付款渠道实测反馈
- 风控触发的新场景

不接受的:广告软文、未经验证的二手信息、纯 AI 生成的洗稿内容。

## License

[MIT](./LICENSE) © 2026 siuserxiaowei

---

## 关于维护方

这份指南由 [payforgpt.com](https://payforgpt.com) 团队维护。我们运营一个面向中文用户的 AI 订阅代充站,日常踩坑最多,所以把经验整理成开源文档放出来。如果你看完指南还是搞不定付款,或者懒得自己折腾海外卡 / 礼品卡 / 虚拟卡,可以直接来 https://payforgpt.com 看一眼——支持中文客服、支付宝/微信/USDT 付款、卡密自动发货,90% 的订单 5 分钟内到账。

不强卖,看到合适再买。

<!-- SIUSER-CONTACT:START -->

## 联系我 / Contact

想交流 AI 工具、内容自动化、SEO、私域增长或项目合作，可以扫码加我微信。

For collaboration on AI tools, content automation, SEO, private-domain growth, or product experiments, scan the WeChat QR code below.

<img src="https://raw.githubusercontent.com/siuserxiaowei/siuserxiaowei/main/assets/contact/wechat-qrcode.jpg" width="180" alt="WeChat QR code / 微信二维码" />

**关键词 / Keywords**: awesome list, AI SaaS, AI subscription, AI tools, AI tools, AI automation, GitHub Pages, SEO

<!-- SIUSER-CONTACT:END -->
